...
אפליקציית Web חד-פעמית לסדנת "מטעינים מחדש" — פסגה פתח תקווה, **26.4.2026**, ~40 מנהלי בתי ספר (10 קבוצות של 4). האפליקציה נכנסת דרך לינק וואטסאפ (בלי הורדה), מנחה את הקבוצות בעבודה עצמאית, ובמקביל מהווה הדגמה (**Modeling**) של AI ככלי הנחיה. הסדנה חד-פעמית — אחרי 26.4 האפליקציה תושבת.
...
# סקריפט סיכום (ANTHROPIC_API_KEY נדרש בסביבה)
python summarize.py

# פריסה: push ל-main של GitHub repo `TomHagiladi/pitah-tikva-principals` → GitHub Pages → https://tomhagiladi.github.io/pitah-tikva-principals
```
...
- **השיר "לובשת שגרה" בזכויות יוצרים** — המחברת **טלי ורסנו-אייסמן** (לא נועם חורב כפי שהיה במסמך הסיכום המקורי של הפגישה — טעות של AI בסיכום). האפליקציה מציגה בית פתיחה בלבד + כפתור שמפנה ל-yekum.org לקריאה מלאה. אם המחברת תיתן אישור — אפשר להטמיע את הטקסט המלא ב-`#poem-text`.
- **סולי הסוללה דמות מסחרית** של סיגל אלבז ([souly.co.il](https://www.souly.co.il)). האפליקציה משתמשת במד סוללה מקורי שנבנה ב-CSS טהור (לא מעתיקה). אסור להטמיע את האיור המקורי בלי רישיון.
- **`<input type="file" accept="image/*" capture="environment">`** — במובייל פותח מצלמה ישירות. בדסקטופ זה גלריית קבצים רגילה. התנהגות שונה ב-iOS Safari מדי פעם — בדיקה נדרשת לפני הסדנה.
...
- **Claude API** — הסיכום של המשובים דורש `summarize.py` לרוץ על הלפטופ של תום בזמן הסדנה, עם `ANTHROPIC_API_KEY` ב-env. המפתח **לא** מוטמע ב-index.html (זה client-side וייחשף).
- אחרי 26.4.2026 אפשר למחוק את פרויקט Firebase לגמרי.
...
2. הדשבורד כותב ל-`/summaryRequest` timestamp חדש
3. `summarize.py` (רץ בטרמינל על הלפטופ של תום) בודק את הנתיב הזה כל 3 שניות
4. ברגע שהוא רואה timestamp חדש — קורא את `/feedback`, שולח ל-Claude (sonnet-4-6), וכותב את התוצאה ל-`/summary`
5. הדשבורד (פרטי וציבורי) מאזין ל-`/summary` ומציג את הסיכום אוטומטית

**חייבים:** הסקריפט חייב לרוץ לפני שתום לוחץ על הכפתור. אם הסקריפט לא רץ — הבקשה נכתבת ל-DB אבל אף אחד לא מעבד אותה. הדשבורד מציג את ההודעה "ממתין" לנצח.
